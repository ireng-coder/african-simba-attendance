...
export default function App() {
  ...
  const masterQrUrl = window.location.href;

  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', display: 'flex', flexDirection: 'column', fontFamily: 'sans-serif' }}>
      {/* Header */}
      <header style={{ backgroundColor: '#0f172a', color: '#ffffff', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          <button 
            ...
            style={{ padding: '0.5rem 1rem', borderRadius: '6px', border: 'none', backgroundColor: activeTab === 'terminal' ? '#3b82f6' : 'transparent', color: '#ffffff', cursor: 'pointer', fontWeight: '500' }}>
            📲 Gate Terminal
          </button>
          <button 
            onClick={() => setActiveTab('masterQR')}
            style={{ padding: '0.5rem 1rem', borderRadius: '6px', border: 'none', backgroundColor: activeTab === 'masterQR' ? '#3b82f6' : 'transparent', color: '#ffffff', cursor: 'pointer', fontWeight: '500' }}>
            🖨️ Master QR Poster
          </button>
          <button 
            onClick={() => setActiveTab('admin')}
            style={{ padding: '0.5rem 1rem', borderRadius: '6px', border: 'none', backgroundColor: activeTab === 'admin' ? '#3b82f6' : 'transparent', color: '#ffffff', cursor: 'pointer', fontWeight: '500' }}>
            📊 Admin Logs
          </button>
        </div>
      </header>
      ...
      <main style={{ flex: 1, padding: '2rem', maxWidth: '900px', width: '100%', margin: '0 auto' }}>
        {statusMessage && (
          <div style={{ padding: '1rem', marginBottom: '1.5rem', borderRadius: '8px', backgroundColor: statusMessage.type === 'success' ? '#dcfce7' : '#fee2e2', color: statusMessage.type === 'success' ? '#166534' : '#991b1b', display: 'flex', alignItems: 'center', gap: '0.75rem', border: `1px solid ${statusMessage.type === 'success' ? '#bbf7d0' : '#fecaca'}` }}>
        ...
        {activeTab === 'terminal' ? (
          <div style={{ backgroundColor: '#ffffff', padding: '2rem', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0' }}>
            ...
            <form onSubmit={handleSubmitAttendance} style={{ maxWidth: '500px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div>
                ...
                <select 
                  ...
                  style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem', backgroundColor: '#f8fafc' }}
                >
              ...
              <button 
                type="submit"
                disabled={loading}
                style={{ backgroundColor: actionType === 'CHECK_IN' ? '#059669' : '#dc2626', color: '#ffffff', padding: '0.85rem', borderRadius: '8px', border: 'none', fontSize: '1rem', fontWeight: '600', cursor: 'pointer', marginTop: '0.5rem' }}
              >
          ...
          <div style={{ backgroundColor: '#ffffff', padding: '2.5rem', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', border: '1px solid #e2e8f0', textAlign: 'center', maxWidth: '600px', margin: '0 auto' }}>
            ...
            <div style={{ background: '#fff', padding: '2rem', borderRadius: '12px', border: '2px dashed #cbd5e1', display: 'inline-block', marginBottom: '1.5rem' }}>
              <QRCodeSVG value={masterQrUrl} size={220} />
            </div>

            <div style={{ background: '#f8fafc', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0', wordBreak: 'break-all', fontSize: '0.85rem', color: '#475569' }}>
                    ...
                    checkIns.map((record) => {
                      ...
                      return (
                        <tr key={record.id} style={{ borderBottom: '1px solid #f1f5f9', fontSize: '0.9rem', color: '#334155' }}>
                          ...
                          <td style={{ padding: '0.75rem' }}>{new Date(record.timestamp).toLocaleString()}</td>
                        </tr>
                      );
                    })
                  )}
